require(['jquery'], function ($) {
    $(document).ready(function () {
        let parentCategoriesTwo = document.querySelectorAll('.parentCategoriesTwo');

        parentCategoriesTwo.forEach(function ($value) {
            $value.addEventListener('click', function () {
                const collapse = $value.parentElement.querySelector('.collapse');
        ...
        function toggleWishlistProduct(productId) {
            $.ajax({
                url: 'shop/Index/Index', // Replace with the correct route URL
                type: 'POST',
                data: {
                ...
                success: function (response) {
                    ...
                    if (response.redirect_url) {
                        ...
                        // Toggle the product in the local wishlist_products array
                        if (wishlist_products.includes(productId)) {
                            // Product is in the wishlist, remove it
                            wishlist_products = wishlist_products.filter(id => id !== productId);
        ...
        $.ajax({
            url: 'shop/Index/wishlistItems', // Replace with the correct route URL
            type: 'GET',
            dataType: 'json',
            success: function (response) {
                wishlist_products = response.items || [];

                // Loop through each SVG element with class 'nn'
                $('.nn').each(function () {
                    let productId = parseInt($(this).data('product-id'));

                    // Check if the 'productId' exists in the wishlist_products array
                    if (wishlist_products.includes(productId)) {
